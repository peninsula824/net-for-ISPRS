...
def training(args, num_classes, model, optimizer, train_dataset, train_loader, criterion1, criterion2, device, epoch):
    ...
    for index, data in enumerate(tbar):
        ...
        #print("Targets shape:", masks.shape)
        #print("Targets values:", masks)
    ...
    csv_path = os.path.join(args.directory, 'training_metrics.csv')
    with open(csv_path, 'a', newline='') as csvfile:
        metric_writer = csv.writer(csvfile)

        metric_writer.writerow([
            epoch, train_loss.avg, train_acc, train_F1_score, train_IoU, train_mean_IoU, train_kappa, lr
        ])
    return train_acc
